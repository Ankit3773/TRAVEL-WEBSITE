...
const SHOW_LOCAL_ADMIN_HINT = Boolean(APP_CONFIG.showLocalAdminHint);
const LOCAL_ADMIN_EMAIL = APP_CONFIG.localAdminEmail || '';
const LOCAL_ADMIN_PASSWORD = APP_CONFIG.localAdminPassword || '';

const elements = {
  setupForm: document.getElementById('admin-setup-form'),
  setupName: document.getElementById('admin-setup-name'),
  setupEmail: document.getElementById('admin-setup-email'),
  setupPassword: document.getElementById('admin-setup-password'),
  setupStatus: document.getElementById('admin-setup-status'),
  ...
  localHint: document.getElementById('admin-local-hint'),
  ...
  qualityRefresh: document.getElementById('admin-quality-refresh'),
  qualitySummary: document.getElementById('admin-quality-summary'),
  qualityIssues: document.getElementById('admin-quality-issues'),
  ...
  routeDescription: document.getElementById('admin-route-description'),
  routeHighlights: document.getElementById('admin-route-highlights'),
  routeTips: document.getElementById('admin-route-tips'),
  ...
  scheduleBoardingPoint: document.getElementById('admin-sch-boarding-point'),
  scheduleBoardingNotes: document.getElementById('admin-sch-boarding-notes'),
  scheduleDroppingPoint: document.getElementById('admin-sch-dropping-point'),
  scheduleDroppingNotes: document.getElementById('admin-sch-dropping-notes'),
...
function renderLocalAdminHint() {
  if (!elements.localHint) return;
  if (!SHOW_LOCAL_ADMIN_HINT || !LOCAL_ADMIN_EMAIL || !LOCAL_ADMIN_PASSWORD) {
    elements.localHint.classList.add('hidden');
    return;
  }

  elements.localHint.textContent = `Local default admin login: ${LOCAL_ADMIN_EMAIL} / ${LOCAL_ADMIN_PASSWORD}`;
  elements.localHint.classList.remove('hidden');

  if (!elements.loginEmail.value.trim()) {
    elements.loginEmail.value = LOCAL_ADMIN_EMAIL;
  }
}

async function loadAdminSetupStatus() {
  try {
    state.adminSetup = await api('/api/auth/admin-setup-status');
  } catch (err) {
    state.adminSetup = {
      adminExists: true,
      setupAllowed: false
...
function computeQualityData() {
  const activeRoutes = state.routes.filter(route => route.active);
  const activeBuses = state.buses.filter(bus => bus.active);
  const activeSchedules = state.schedules.filter(schedule => schedule.active);
  const today = toIsoDate(new Date());

  const upcomingSchedules = activeSchedules.filter(schedule => schedule.travelDate >= today);
  const routesWithUpcoming = new Set(upcomingSchedules.map(schedule => schedule.route.id));
  const busesWithUpcoming = new Set(upcomingSchedules.map(schedule => schedule.bus.id));

  const routesMissingUpcoming = activeRoutes.filter(route => !routesWithUpcoming.has(route.id));
  const busesMissingUpcoming = activeBuses.filter(bus => !busesWithUpcoming.has(bus.id));

  const invalidFareSchedules = activeSchedules.filter(schedule => Number(schedule.baseFare) <= 0);
  const overnightTimingSchedules = activeSchedules.filter(schedule => {
    const departure = minutesFromTime(schedule.departureTime);
    const arrival = minutesFromTime(schedule.arrivalTime);
  ...
  activeRoutes.forEach(route => {
    ...
    const existing = routeKeyMap.get(key) || [];
    existing.push(route);
    routeKeyMap.set(key, existing);
  });
  routeKeyMap.forEach(items => {
    if (items.length > 1) {
      duplicateRouteNames.push(...items);
    }
  });
    ...
    summary: [
      {
        ...
        copy: `${routesMissingUpcoming.length} without an upcoming schedule`
      },
      {
        label: 'Active buses',
        value: activeBuses.length,
        copy: `${busesMissingUpcoming.length} not assigned to upcoming trips`
      },
      {
        label: 'Upcoming schedules',
        value: upcomingSchedules.length,
        copy: `${invalidFareSchedules.length} with invalid fares`
      },
      {
        label: 'Quality alerts',
        value: routesMissingUpcoming.length + busesMissingUpcoming.length + invalidFareSchedules.length + overnightTimingSchedules.length + duplicateRouteNames.length,
        copy: 'Use the issue cards below to clean up registry gaps'
      }
    ],
    issues: [
      {
        title: 'Routes missing upcoming trips',
        count: routesMissingUpcoming.length,
        emptyCopy: 'Every active route has at least one upcoming schedule.',
        items: routesMissingUpcoming.map(route => `${route.source} -> ${route.destination} | ${route.distanceKm} km`)
      },
      {
        title: 'Buses not used in upcoming trips',
        count: busesMissingUpcoming.length,
        emptyCopy: 'Every active bus is assigned to an upcoming schedule.',
        items: busesMissingUpcoming.map(bus => `${bus.busNumber} | ${bus.busType} | ${bus.totalSeats} seats`)
      },
      {
        title: 'Schedules needing fare review',
        count: invalidFareSchedules.length,
        emptyCopy: 'All active schedules currently have a positive fare.',
        items: invalidFareSchedules.map(schedule =>
          `#${schedule.id} | ${schedule.route.source} -> ${schedule.route.destination} | ${formatDisplayDate(schedule.travelDate)}`
        )
      },
      {
        title: 'Schedules with timing conflicts',
        count: overnightTimingSchedules.length,
        emptyCopy: 'No active schedule has arrival earlier than departure.',
        items: overnightTimingSchedules.map(schedule =>
          `#${schedule.id} | ${schedule.bus.busNumber} | ${formatTime(schedule.departureTime)} -> ${formatTime(schedule.arrivalTime)}`
        )
      },
      {
        title: 'Potential duplicate route entries',
        count: duplicateRouteNames.length,
        emptyCopy: 'No duplicate active route names were found.',
        items: duplicateRouteNames.map(route => `#${route.id} | ${route.source} -> ${route.destination}`)
      }
    ]
  };
}
...
function renderQualityPanel() {
  if (!elements.qualitySummary || !elements.qualityIssues) return;
  const quality = computeQualityData();

  elements.qualitySummary.innerHTML = quality.summary.map(item => `
    <article class="metric panel-metric quality-card">
      <div class="label">${item.label}</div>
      <div class="value">${item.value}</div>
      <div class="monitor-copy">${item.copy}</div>
    </article>
  `).join('');

  elements.qualityIssues.innerHTML = quality.issues.map(issue => `
    <article class="quality-issue-card">
      <div class="row">
        <strong>${issue.title}</strong>
        <span class="info-pill">${issue.count} alert${issue.count === 1 ? '' : 's'}</span>
      </div>
      ${issue.items.length
        ? `<ul class="quality-issue-list">${issue.items.map(item => `<li>${item}</li>`).join('')}</ul>`
        : `<p class="hint">${issue.emptyCopy}</p>`}
    </article>
  `).join('');
}
...
function editRoute(route) {
  ...
  elements.routeHighlights.value = route.travelHighlights || '';
  elements.routeTips.value = route.travelTips || '';
...
function editSchedule(schedule) {
  ...
  elements.scheduleBoardingPoint.value = schedule.boardingPoint || '';
  elements.scheduleBoardingNotes.value = schedule.boardingNotes || '';
  elements.scheduleDroppingPoint.value = schedule.droppingPoint || '';
  elements.scheduleDroppingNotes.value = schedule.droppingNotes || '';
...
function renderRouteList() {
  ...
  elements.routeList.innerHTML = state.routes.map(route => `
    <article class="item manager-item">
      ...
      <p class="muted">${route.description || 'Public route summary will appear after content is added.'}</p>
...
function renderScheduleList() {
  ...
  elements.scheduleList.innerHTML = state.schedules.map(schedule => `
    <article class="item manager-item">
      ...
      <div class="row muted">
        <span>${schedule.route.distanceKm} km | ${schedule.boardingPoint || 'Boarding point auto-managed'}</span>
        <span>Fare ${formatAmount(schedule.baseFare)}</span>
      </div>
      <div class="row muted">
        <span>Drop: ${schedule.droppingPoint || 'Drop point auto-managed'}</span>
        <span>${schedule.boardingNotes || 'Boarding notes auto-generated'}</span>
      </div>
      <div class="row manager-actions">
        <button type="button" data-edit-schedule="${schedule.id}">Edit schedule</button>
...
function renderAccessState() {
  ...
  const setupAllowed = Boolean(state.adminSetup?.setupAllowed);

  elements.logoutBtn.classList.toggle('hidden', !loggedIn);

  if (setupAllowed) {
    elements.authSummary.textContent = loggedIn ? `Logged in as ${user.name} (${user.role})` : 'Admin account not created yet.';
    elements.accessTitle.textContent = 'Create the first admin';
    elements.accessCopy.textContent = 'No admin account exists yet. Create the first admin here to unlock route, schedule, fare, and booking management.';
    setSummary(elements.accessStatus, 'First admin setup is available.');
...
elements.setupForm.addEventListener('submit', async event => {
  ...
  try {
    const payload = {
      ...
      email: elements.setupEmail.value.trim(),
      password: elements.setupPassword.value
    };
    const data = await api('/api/auth/setup-first-admin', { method: 'POST', body: JSON.stringify(payload) });
    setAuth(data);
    elements.setupForm.reset();
    setSummary(elements.setupStatus, 'First admin created successfully.', 'ok');
    await syncAccessState();
  } catch (err) {
    setSummary(elements.setupStatus, err.message, 'err');
  }
});
...
if (elements.qualityRefresh) {
  elements.qualityRefresh.addEventListener('click', () => {
    if (isAdmin()) {
      refreshAdminData({ keepBookingPage: true });
    }
  });
}
...
elements.routeForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    const payload = {
      ...
      description: elements.routeDescription.value.trim(),
      travelHighlights: elements.routeHighlights.value.trim(),
      travelTips: elements.routeTips.value.trim(),
...
elements.scheduleForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    const payload = {
      ...
      boardingPoint: elements.scheduleBoardingPoint.value.trim(),
      boardingNotes: elements.scheduleBoardingNotes.value.trim(),
      droppingPoint: elements.scheduleDroppingPoint.value.trim(),
      droppingNotes: elements.scheduleDroppingNotes.value.trim(),
